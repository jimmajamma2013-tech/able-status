...
- publicMirrorReadable: true (unauthenticated fetch of the raw mirror, 2026-09-17T17:49:12.607Z)
...
- THE INTEGRATED FOLD ④ @ 0511c6e8d9 on fold-int-20260916 (pushed) — lane A + fold-bc + Bandcamp absorbed, lane F merged, F26 closed (the landscape title never breaks a word: 26 red on the parent → 0; F25 and F24 probes green; tsc 0; vitest 1069/1069; eye-check 0 P0 at three sizes; the divisor is fitted to this title's longest word — a longer word would overflow, named not closed; the desktop title 3% smaller, unruled). VERDICT RUNNING on C (Opus). NOT claimed: DUAL at 834 and WIDE at 1440 on the Work.
...
- the integrated fold's verdict lands (C, Opus) → PASS folds ④ to canonical by merge (c-census and c-identity fold with it; the two receipts re-run) → backpressure drops
